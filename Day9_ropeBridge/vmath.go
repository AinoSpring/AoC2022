...
func (matrix Matrix) Multiplication(other Vector) (multipliedVector Vector) {
	multipliedVector = NewVector(matrix.Size()[1])
	for idx, vector := range matrix {
		multipliedVector[idx] = vector.Mul(multipliedVector).Sum()
	}
	return
}
...
func (matrix Matrix) Count(other Vector) (count int) {
	for _, vector := range matrix {
		if vector.Equal(other) {
			count++
		}
	}
	return
